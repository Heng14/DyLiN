...
Example: 
        ...
        python convert_original_data_to_rays_blender.py --splits train --datadir data/lego
"""
...
all_imgs, all_poses, all_time, metas = [], [], [], {}
...
for s in splits:
    ...
    for frame in meta['frames']:
        ...
        if img_index not in ignored_img_indices:
            ...
            times.append(np.array(frame['time']))
...
all_time = np.concatenate(all_time, 0)
print(
    f'Read all images and poses, done. all_imgs shape {all_imgs.shape}, all_poses shape {all_poses.shape}, all_times shape {all_time.shape}'
)
...
for im, po, t in zip(all_imgs, all_poses, all_time):
    if args.donerf:
    ...
    t = t.repeat([400, 400, 3])
    data = torch.cat([rays_o, rays_d, t, im], dim=-1)  # [H, W, 12]
    # print(data.shape)
    # raise
    all_data += [data.view(H * W, 12)]  # [H*W, 9]
