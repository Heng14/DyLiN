...
from model.nerf_raybased import NeRF, NeRF_v3_2, PositionalEmbedder, PointSampler, Net_offset, Net_hyper
...
def render_path(render_poses, test_times,
                hwf,
                chunk,
                render_kwargs,
                gt_imgs=None,
    ...
    render_kwargs['network_fn_offset'].eval()
    render_kwargs['network_fn_hyper'].eval()
    ...
    if args.hyperdata:
        gt_imgs = []
        # test_dir = '/home/hengyu/projects/R2L_heng_LearnOffset_hyper/data/nerf_synthetic/vrig-chicken_0203_hypernerf_test'
        # test_dir = '/home/hengyu/projects/R2L_heng_LearnOffset/data/nerf_synthetic/vrig-chicken_0203_hypernerf_real_test'
        test_dir = '/home/hengyu/projects/R2L_heng_LearnOffset/data/nerf_synthetic/vrig-chicken_0203_hypernerf_test_demo'
        # test_dir = '/home/hengyu/projects/R2L_heng_LearnOffset/data/nerf_synthetic/captures_7361_hypernerf_test_demo'
        # test_dir = '/home/hengyu/projects/R2L_heng_LearnOffset/data/nerf_synthetic/americano_0130_hypernerf_test_campath'
        # test_dir = '/home/hengyu/projects/R2L_heng_LearnOffset/data/nerf_synthetic/americano_0130_hypernerf_test_demo'
        # test_dir = '/home/hengyu/projects/R2L_heng/data/nerf_synthetic/americano_0130_hypernerf_real_test'
        # test_dir = '/home/hengyu/projects/R2L_heng/data/nerf_synthetic/3dprinter++_hypernerf_real_test'
        # test_dir = '/home/hengyu/projects/R2L_heng/data/nerf_synthetic/broom++_hypernerf_real_test'
        # test_dir = '/home/hengyu/projects/R2L_heng/data/nerf_synthetic/banana++_hypernerf_real_test'
        H, W = 480, 268
        # test_dir = '/home/hengyu/projects/R2L_heng_LearnOffset/data/nerf_synthetic/captures_7361_hypernerf_test'
        # test_dir = '/home/hengyu/projects/R2L_heng/data/nerf_synthetic/captures_7361_hypernerf_real_test'
        # H, W = 480, 270

        test_list = glob.glob(f'{test_dir}/*.npy')
        test_list.sort()

        H_, W_ = H, W
        # for i, test_npy in enumerate(glob.glob(f'{test_dir}/*.npy')):

        if True: #False: # true for explore hyperspace 
            hyper_value = np.linspace(-0.2, 0.2, num=20)

            # for i, test_npy in enumerate(test_list):
            for i in range(len(hyper_value)):
                t0 = time.time()
                test_i = np.load(test_list[100])
                test_i = np.float32(test_i)
                # test_i = test_i.reshape(H, W, -1)

                rays_o = torch.from_numpy(test_i[..., :3]).to(device)
                rays_d = torch.from_numpy(test_i[..., 3:6]).to(device)
                test_t = torch.from_numpy(test_i[..., 6:7]).to(device)
                gt_img = torch.from_numpy(test_i[..., 9:]).to(device)
                gt_img = gt_img.reshape(H, W, -1)
                gt_imgs.append(gt_img)

                model = render_kwargs['network_fn']
                model_offset = render_kwargs['network_fn_offset']
                model_hyper = render_kwargs['network_fn_hyper']
                ...
                with torch.no_grad():
                    ...
                    # pts = pts.reshape(-1, args.n_sample_per_ray * pts.shape[-1])

                    # if test_times == None:
                    #     test_t = torch.rand(1).to(device)
                    # else:
                    #     test_t = test_times[i] #torch.rand(1).to(device)
                    # test_t = test_t.repeat([pts.shape[0], 1])


                    rays_odt = torch.cat([rays_o, rays_d, test_t], dim=-1)
                    offset_o, offset_d = model_offset(positional_embedder(rays_odt)) 
                    rays_hyper = model_hyper(positional_embedder(rays_odt)) 

                    # rays_hyper[:, :] = hyper_value[i] # by heng 
                    print (rays_hyper.max(),  rays_hyper.min())
                    print (rays_hyper.shape)
                    tmp = to_array(rays_hyper.reshape(H, W, -1))
                    print (tmp.shape)
                    for ch_i in range(8):
                        imsave(f'test_hyperspace_{ch_i}.png', tmp[..., ch_i])
                    raise

                    pts = point_sampler.sample_train(rays_o + offset_o,
                                                    rays_d + offset_d,
                                                    perturb=0)
                    ...
                    pts = torch.cat([pts, rays_hyper], dim=-1)

                    model_input = positional_embedder(pts) # by heng 
                    ...
                    if args.learn_depth:
                        ...
                        rgb = model(model_input) #, offset_gt)

                        # pts_raw = model(model_input)
                        # pts_raw = pts_raw.view(-1, args.n_sample_per_ray, pts_raw.shape[-1])       
                        # rgb, disp_map, acc_map, weights, depth_map = rendering_pts(pts_raw, pts_z_vals, rays_d)
                ...
                if gt_img is not None:
                    errors += [(rgb - gt_img[:H, :W, :]).abs()]
                    psnrs += [mse2psnr(img2mse(rgb, gt_img[:H, :W]))]
                    # ssims += [ssim(rgb, gt_img[:H, :W])]

                    rgb_ms = rgb.permute(2, 0, 1)
                    gt_img_ms = gt_img[:H, :W].permute(2, 0, 1)
                    ssims += [ms_ssim(rgb_ms[None, ...], gt_img_ms[None, ...], data_range=1, size_average=True )] # ms-ssim by heng
                ...
                with torch.no_grad():
                    if args.given_render_path_rays:  # To test DONERF data using our model
                        ...
                        if args.plucker:
                            ...
                            pts, rays_o, rays_d = point_sampler.sample_test(
                    ...
                    # rays_hyper = rays_hyper * 0 # by heng 
    ...
    render_kwargs['network_fn_offset'].train()
    render_kwargs['network_fn_hyper'].train()
...
def render_path_fix_pose(render_poses, test_times,
                hwf,
                chunk,
                render_kwargs,
                gt_imgs=None,
    ...
    if args.hyperdata:
        gt_imgs = []
        # test_dir = '/home/hengyu/projects/R2L_heng_LearnOffset_hyper/data/nerf_synthetic/vrig-chicken_0203_hypernerf_test_campath'
        # test_dir = '/home/hengyu/projects/R2L_heng_LearnOffset/data/nerf_synthetic/americano_0130_hypernerf_test_campath'
        # H, W = 480, 268
        # test_dir = '/home/hengyu/projects/R2L_heng_LearnOffset/data/nerf_synthetic/captures_7361_hypernerf_test_campath'
        test_dir = '/home/hengyu/projects/R2L_heng/data/nerf_synthetic/captures_7361_hypernerf_real_test'
        H, W = 480, 270
        ...
        npy_list = glob.glob(f'{test_dir}/*.npy')
        print ('test name: ', npy_list[test_id])

        test_i = np.load(npy_list[test_id])
        
        for i, test_t in enumerate(test_times):
            ...
            rays_o = torch.from_numpy(test_i[..., :3]).to(device)
            rays_d = torch.from_numpy(test_i[..., 3:6]).to(device)
            test_t_ori = torch.from_numpy(test_i[..., 6:7]).to(device)
            gt_img = torch.from_numpy(test_i[..., 9:]).to(device)
            gt_img = gt_img.reshape(H, W, -1)
            gt_imgs.append(gt_img)
        ...
        for i, test_t in enumerate(test_times):
            ...
            if args.model_name in ['nerf']:
                ...
                with torch.no_grad():
                    ...
                    test_t = test_t.repeat([pts.shape[0], 1])
                    pts = torch.cat([pts, test_t], dim=-1)
...
def create_nerf(args, near, far):
    ...
    if args.model_name in ['nerf']:
        ...
        if args.plucker:
            ...
            # input_dim0 = (args.n_sample_per_ray * 3 + 1) * positional_embedder.embed_dim

            input_offset_dim = (3 + 3 + 1) * positional_embedder.embed_dim #rays_o, rays_d, t
            hyper_dim = 8 #8 #2 #24 
            input_dim = (args.n_sample_per_ray * 3 + hyper_dim) * positional_embedder.embed_dim

        model = NeRF_v3_2(args, input_dim, 3).to(device)
        model_offset = Net_offset(input_ch=input_offset_dim, output_ch=3).to(device)
        model_hyper = Net_hyper(input_ch=input_offset_dim, output_ch=hyper_dim).to(device)

        if not args.freeze_pretrained:
            grad_vars += list(model.parameters())
            grad_vars += list(model_offset.parameters())
            grad_vars += list(model_hyper.parameters())
    ...
    # use DataParallel
    if not args.render_only and not args.render_only_fix_pose:  # when rendering, use just one GPU
        model = MyDataParallel(model)
        model_offset = MyDataParallel(model_offset)
        model_hyper = MyDataParallel(model_hyper)
    ...
    if args.pretrained_ckpt:
        ...
        if 'network_fn' in ckpt:
            ...
            model_offset = ckpt['network_fn_offset']
            model_hyper = ckpt['network_fn_hyper']
            grad_vars = list(model.parameters())
            grad_vars += list(model_offset.parameters())
            grad_vars += list(model_hyper.parameters())
    ...
    render_kwargs_train = {
        ...
        'network_fn': model,
        'network_fn_offset':model_offset,
        'network_fn_hyper':model_hyper,
...
def render_rays(ray_batch,
                network_fn,
                network_fn_offset, network_fn_hyper,
                network_query_fn,
                N_samples,
                retraw=False,
                lindisp=False,
...
def train():
    ...
    if args.dataset_type == 'llff':
        ...
        images, poses, times, render_poses, hwf, i_split = load_blender_data(
    ...
    if args.test_pretrained:
        ...
        with torch.no_grad():
            *_, misc = render_path(test_poses, test_times, 
                                   hwf,
                                   4096,
                                   render_kwargs_test,
                                   gt_imgs=test_images,
    ...
    if args.render_only:
        ...
        with torch.no_grad():
            ...
            if args.render_test:
                ...
                rgbs, *_, misc = render_path(test_poses, test_times, # test_poses, test_times
                                             hwf,
                                             args.chunk,
                                             render_kwargs_test, # render_kwargs_test
                                             gt_imgs=test_images, #test_images
                ...
                rgbs, *_, misc = render_path(video_poses, None, 
                                             hwf,
                                             args.chunk,
                                             render_kwargs_test,
    ...
    if args.render_only_fix_pose:
        print('RENDER ONLY fix pose')
        expid, iter_ = parse_expid_iter(args.pretrained_ckpt)

        pose_id = 74
        # render_poses = np.array(poses[pose_id])
        # render_times = np.array(times[:50])
        render_times = np.arange(0, 1, 0.02)

        # render_poses = torch.Tensor(render_poses).to(device)
        render_times = torch.Tensor(render_times).to(device)

        render_poses = poses[pose_id]

        with torch.no_grad():
            # images = images[pose_id]

            rgbs, *_, misc = render_path_fix_pose(render_poses, render_times, 
                                            hwf,
                                            args.chunk,
                                            render_kwargs_test,
                                            gt_imgs=None, 
        ...
        video_path = f'{logger.gen_img_path}/video_{expid}_iter{iter_}_{args.video_tag}_pose{pose_id}.mp4'
        imageio.mimwrite(video_path, to8b(rgbs), fps=30, quality=8)
        if 'errors' in misc:
            imageio.mimwrite(video_path.replace('.mp4', '_error.mp4'),
                             to8b(misc['errors']),
    ...
    for i in trange(start + 1, args.N_iters + 1):
        ...
        if use_batching:  # @mst: False in default
            ...
            if N_rand is not None:
                if args.data_mode in ['images']:
                    ...
                    rays_o, rays_d, rays_t, target_s = trainloader.next(
            ...
            if hard_pool_full:
                ...
                rays_d = torch.cat([rays_d, picked_hard_rays[:, 3:3 + 3]],
                                   dim=0)
                rays_t = torch.cat([rays_t, picked_hard_rays[:, 3 + 3:3 + 3 + 1]],
                                   dim=0)

                target_s = torch.cat([target_s, picked_hard_rays[:, 3 + 3 + 1:]],
                                     dim=0)
        ...
        if args.model_name == 'nerf':
            ...
            model_offset = render_kwargs_train['network_fn_offset']
            model_hyper = render_kwargs_train['network_fn_hyper']
            perturb = render_kwargs_train['perturb']


            rays_odt = torch.cat([rays_o, rays_d, rays_t], dim=-1)
            offset_o, offset_d = model_offset(positional_embedder(rays_odt)) 
            rays_hyper = model_hyper(positional_embedder(rays_odt)) 
        ...
        if args.hard_ratio:
            ...
            hard_rays_ = torch.cat([
                rays_o[hard_indices], rays_d[hard_indices], rays_t[hard_indices],
...
def save_ckpt(file_name, render_kwargs_train, optimizer, best_psnr,
    ...
    if args.model_name in ['nerf_v3.2', 'R2L']:
        ...
        to_save['network_fn_offset'] = undataparallel(
            render_kwargs_train['network_fn_offset'])
        to_save['network_fn_hyper'] = undataparallel(
            render_kwargs_train['network_fn_hyper'])
