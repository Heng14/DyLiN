...
class PointSampler():
    ...
    def sample_test(self, c2w):  # c2w: [3, 4]
        ...
        return pts.view(pts.shape[0], -1), rays_o, rays_d
...
# class Net_offset(nn.Module):
#     '''Based on NeRF_v3, move positional embedding out'''

#     def __init__(self, args, input_dim, output_dim):
#         super(Net_offset, self).__init__()
#         self.args = args
#         _, W = args.netdepth, args.netwidth
#         D = 5
...
class Net_offset(nn.Module):
    ...
    def __init__(self, D=7, W=128, input_ch=3, output_ch=4, skips=[4]):
        """ 
        """
        super(Net_offset, self).__init__()
        self.D = D
...
class Net_hyper(nn.Module):
    ...
    def __init__(self, D=6, W=64, input_ch=3, output_ch=4, skips=[4]):
        """ 
        """
